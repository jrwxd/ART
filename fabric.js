...
let TIME_STEP = 0.016;       // Simulation time step
...
function init() {
    ...
    // Camera
    camera = new THREE.PerspectiveCamera(75, window.innerWidth / window.innerHeight, 0.011, 10000000);
...
function createUI() {
    ...
    uiContainer.appendChild(createSlider('Particle Count', 5, 100, PARTICLE_COUNT, 1, (value) => {
        PARTICLE_COUNT = value;
    }));
    
    uiContainer.appendChild(createSlider('Graph Density', 0.1, 1.0, GRAPH_DENSITY, 0.05, (value) => {
        GRAPH_DENSITY = value;
    }));
    
    uiContainer.appendChild(createSlider('Grid Spacing', 0.1, 10, GRID_SPACING, 0.1, (value) => {
        GRID_SPACING = value;
    }));
    
    uiContainer.appendChild(createSlider('Spring Stiffness', 1, 1000, SPRING_K_BASE, 1, (value) => {
        SPRING_K_BASE = value;
        // Update all constraints to use the new spring constant
        constraints.forEach(c => c.k = value * (c.k === SPRING_K_BASE * 0.7 ? 0.7 : 1));
    }));
    
    uiContainer.appendChild(createSlider('Damping', 0, 8, DAMPING, 0.01, (value) => {
        DAMPING = value;
    }));
    
    uiContainer.appendChild(createSlider('Gravity', -20, 20, GRAVITY.y, 0.1, (value) => {
        GRAVITY.y = value;
    }));
    
    uiContainer.appendChild(createSlider('Mass', 1, 500, PARTICLE_MASS, 1, (value) => {
        PARTICLE_MASS = value;
    ...
    uiContainer.appendChild(createSlider('Physics Steps', 1, 20, STEPS_PER_FRAME, 1, (value) => {
        STEPS_PER_FRAME = value;
    }));
    
    // Add checkbox for pinned corners
    uiContainer.appendChild(createCheckbox('Pin Corners', PINNED_PARTICLES.length > 0, (value) => {
    ...
    buttonContainer.appendChild(createButton('Reset', () => {
    ...
    buttonContainer.appendChild(createButton('Redraw', () => {
        resetSimulation();
    }));
    
    // Add a button to regenerate the graph
    buttonContainer.appendChild(createButton('Regenerate Graph', () => {
        createGraphBasedSystem();
    }));
